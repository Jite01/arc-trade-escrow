...
export const acceptCommercialProposal = (session: EmbeddedWalletSession, id: string) => request<{ proposal: CommercialProposal; agreed: boolean; agreedMilestones: CommercialMilestone[] | null }>(`/agreements/${encodeURIComponent(id)}/accept`, session, { method: "POST", body: "{}" });
...
export const recordCommercialDeployment = (session: EmbeddedWalletSession, id: string, contractAddress: string, deploymentBlock: number) => request<CommercialAgreement>(`/agreements/${encodeURIComponent(id)}/deploy`, session, { method: "POST", body: JSON.stringify({ contractAddress, deploymentBlock }) });
