...
function Dashboard(props: { session: EmbeddedWalletSession; company: Company | null; profileHint: string; openProposals: Proposal[]; myProposals: Proposal[]; agreements: AgreementRecord[]; busy: string; message: string; proposalLink: string; recipientCompany: string; agreementLabel: string; proposalDescription: string; proposalVisibility: ProposalVisibility; sellerCommitmentHours: string; buyerResponseMinutes: string; disputeWindowMinutes: string; proposalLifetimeHours: string; joinId: string; setRecipientCompany: (value: string) => void; setAgreementLabel: (value: string) => void; setProposalDescription: (value: string) => void; setProposalVisibility: (value: ProposalVisibility) => void; setSellerCommitmentHours: (value: string) => void; setBuyerResponseMinutes: (value: string) => void; setDisputeWindowMinutes: (value: string) => void; setProposalLifetimeHours: (value: string) => void; setJoinId: (value: string) => void; create: () => void; join: () => void; removeExpired: (proposal: Proposal) => void; restoreProfile: (name: string) => void; select: (record: AgreementRecord) => void; signOut: () => void; }) {
  ...
  return <main className="shell workspace-shell">
    <header className="site-header"><a className="brand" href="/"><span className="mark">AT</span><span>Arc<span>Trade</span></span></a><div className="account"><strong>{props.company?.name || "Company profile"}</strong><a className="commercial-nav-link" href="/agreements/new">New commercial agreement</a></div><button className={`menu-toggle${menuOpen ? " is-open" : ""}`} aria-label="Open navigation" aria-expanded={menuOpen} onClick={() => setMenuOpen(open => !open)}><i /><i /></button>{menuOpen && <nav className="menu-panel"><a href="/agreements/new" onClick={() => setMenuOpen(false)}>New commercial agreement</a><a href="#issue" onClick={() => { setMenuOpen(false); jumpTo(".create-panel"); }}>Issue proposal</a><a href="#board" onClick={() => { setMenuOpen(false); jumpTo("#board"); }}>Proposal board</a><a href="#agreements" onClick={() => { setMenuOpen(false); jumpTo(".agreements-panel"); }}>Agreements</a><button className="quiet" onClick={props.signOut}>Sign out</button></nav>}</header>
    ...
    {issued.length > 0 && <ActivityPanel title="Issued proposals" eyebrow="Your outbound activity" proposals={issued} props={props} open={setProposalForJoin} removeExpired={props.removeExpired} />}
    {received.length > 0 && <ActivityPanel title="Received proposals" eyebrow="Your inbound activity" proposals={received} props={props} open={setProposalForJoin} removeExpired={props.removeExpired} />}
