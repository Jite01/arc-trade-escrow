...
function App() {
  ...
  const create = async () => {
    ...
    try {
      ...
      const shareValue = proposal.visibility === "PRIVATE"
        ? `${window.location.origin}/signin/${recipientSlug}/${proposal.id}`
        : proposal.id;
      ...
      setMessage(proposal.visibility === "PRIVATE"
        ...
        : `Public proposal published. Share this proposal ID: ${proposal.id}`);
...
function Dashboard(props: { session: EmbeddedWalletSession; company: Company | null; openProposals: Proposal[]; myProposals: Proposal[]; agreements: AgreementRecord[]; busy: string; message: string; proposalLink: string; recipientCompany: string; agreementLabel: string; proposalDescription: string; proposalVisibility: ProposalVisibility; sellerCommitmentHours: string; buyerResponseMinutes: string; disputeWindowMinutes: string; proposalLifetimeHours: string; joinId: string; setRecipientCompany: (value: string) => void; setAgreementLabel: (value: string) => void; setProposalDescription: (value: string) => void; setProposalVisibility: (value: ProposalVisibility) => void; setSellerCommitmentHours: (value: string) => void; setBuyerResponseMinutes: (value: string) => void; setDisputeWindowMinutes: (value: string) => void; setProposalLifetimeHours: (value: string) => void; setJoinId: (value: string) => void; create: () => void; join: () => void; select: (record: AgreementRecord) => void; signOut: () => void; }) {
  ...
  return <main className="shell"><header><span className="mark">AT</span><span>Arc Trade</span><span className="account">{props.company?.name || "Company profile"} · {ref(props.session.address)} <button className="quiet" onClick={props.signOut}>Sign out</button></span></header>{props.message && <div className="error">{props.message}</div>}{props.proposalLink && <section className="panel invitation"><p className="eyebrow">{props.proposalVisibility === "PRIVATE" ? "Invitation ready" : "Proposal published"}</p><h2>{props.proposalVisibility === "PRIVATE" ? "Share this private invitation" : "Copy this proposal ID"}</h2><div className="share-row"><code>{props.proposalLink}</code><button onClick={() => void copyProposalLink()}>{props.proposalVisibility === "PRIVATE" ? "Copy link" : "Copy ID"}</button></div></section>}<div className="topline"><div><p className="eyebrow">Agreement registry</p><h1>Your agreements</h1></div><span className="pill">{props.agreements.length} agreement{props.agreements.length === 1 ? "" : "s"}</span></div><FundingPanel address={props.session.address} /><section className="panel"><div className="section-head"><div><p className="eyebrow">Open board</p><h2>Public proposals</h2></div><span className="pill">{props.openProposals.length} open</span></div>{props.openProposals.length === 0 ? <p className="notice">No public proposals are currently available.</p> : <div className="proposal-list">{props.openProposals.map(proposal => <article className="public-proposal" key={proposal.id}><div><h3>{proposal.title}</h3><p className="notice">{proposal.description || "Documentary trade agreement"}</p><small>{proposalAmount(proposal)} test funds · closes {new Date(proposal.proposalExpiresAt * 1000).toLocaleString()}</small></div><button className="secondary" onClick={() => { props.setJoinId(proposal.id); window.scrollTo({ top: document.body.scrollHeight, behavior: "smooth" }); }}>Open proposal</button></article>)}</div>}</section>{props.myProposals.length > 0 && <section className="panel"><div className="section-head"><div><p className="eyebrow">Your proposals</p><h2>Incoming and outgoing</h2></div><span className="pill">{props.myProposals.length}</span></div><div className="proposal-list">{props.myProposals.map(proposal => <article className="public-proposal" key={proposal.id}><div><h3>{proposal.title}</h3><p className="notice">{proposal.visibility === "PUBLIC" ? "Public proposal" : `Private invitation · ${proposal.recipientCompany || "recipient"}`}</p><small>{proposal.status} · {proposal.agreementId ? `Agreement ${ref(proposal.agreementId)}` : "Awaiting agreement deployment"}</small></div><button className="secondary" onClick={() => { props.setJoinId(proposal.visibility === "PUBLIC" ? proposal.id : `${window.location.origin}/signin/${proposal.recipientCompany || "public"}/${proposal.id}`); window.scrollTo({ top: document.body.scrollHeight, behavior: "smooth" }); }}>{proposal.status === "OPEN" ? "Open" : proposal.agreementId ? "View agreement" : "Deploy agreement"}</button></article>)}</div></section>}<section className="grid terms"><div className="panel"><p className="eyebrow">Proposal flow</p><h2>Propose an agreement</h2><p className="notice">Create a private invitation or publish a public proposal.</p><select aria-label="Proposal visibility" value={props.proposalVisibility} onChange={event => props.setProposalVisibility(event.target.value as ProposalVisibility)}><option value="PRIVATE">Private invitation</option><option value="PUBLIC">Public proposal</option></select>{props.proposalVisibility === "PRIVATE" && <input placeholder="Recipient company" value={props.recipientCompany} onChange={event => props.setRecipientCompany(event.target.value)} />}<input placeholder="Proposal title" value={props.agreementLabel} onChange={event => props.setAgreementLabel(event.target.value)} /><textarea placeholder="What is being traded?" value={props.proposalDescription} onChange={event => props.setProposalDescription(event.target.value)} /><div className="grid terms"><label>Proposal response window (hours)<input type="number" min="1" max="168" value={props.sellerCommitmentHours} onChange={event => props.setSellerCommitmentHours(event.target.value)} /></label><label>Negotiation window (minutes)<input type="number" min="1" max="1440" value={props.buyerResponseMinutes} onChange={event => props.setBuyerResponseMinutes(event.target.value)} /></label><label>Dispute window (minutes)<input type="number" min="1" max="10080" value={props.disputeWindowMinutes} onChange={event => props.setDisputeWindowMinutes(event.target.value)} /></label><label>Proposal stays open (hours)<input type="number" min="1" max="168" value={props.proposalLifetimeHours} onChange={event => props.setProposalLifetimeHours(event.target.value)} /></label></div><button disabled={!!props.busy} onClick={props.create}>{props.busy === "createAgreement" ? "Preparing proposal…" : "Create proposal"}</button></div><div className="panel"><p className="eyebrow">Join flow</p><h2>Join proposal</h2><p className="notice">Paste the proposal ID or invite link shared by the proposer, or select a proposal already indexed to this account.</p><input placeholder="Proposal ID or invite link" value={props.joinId} onChange={event => props.setJoinId(event.target.value)} /><button className="secondary" disabled={!!props.busy} onClick={props.join}>{props.busy === "joinAgreement" ? "Opening…" : "Open proposal"}</button></div></section><section className="panel"><div className="section-head"><div><p className="eyebrow">Registry state</p><h2>My agreements</h2></div><code>{props.session.address}</code></div>{props.agreements.length === 0 ? <p className="notice">No agreements yet. Create one or open an invitation.</p> : <div className="grid">{props.agreements.map(record => <button className="secondary" key={record.id} onClick={() => props.select(record)}>{ref(record.id)} · {ref(record.escrow)} · {record.buyer.toLowerCase() === props.session.address.toLowerCase() ? "INITIATOR" : "COUNTERPARTY"}</button>)}</div>}</section></main>;
}
...
function FundingPanel({ address }: { address: string }) {
  ...
  return <section className="panel funding"><div><p className="eyebrow">Testnet funding</p><h2>Prepare this profile</h2><p className="notice">Circle Faucet requires a Circle account. This account needs test funds; sponsored transactions normally cover fees.</p>{balance !== null && <p className="notice">Available test funds: {balance}</p>}{status && <p className="notice">{status}</p>}</div><div className="actions"><button onClick={() => void faucet()}>Get 20 test funds</button><button className="secondary" onClick={() => void native()}>Get Arc test tokens</button><button className="secondary" onClick={() => void refresh()}>Refresh balance</button></div></section>;
}
