...
function App() {
  ...
  if (!session) return <main className="shell landing-shell">
    ...
    <section className="hero-grid">
      <div className="hero">
        <p className="eyebrow">Documentary commerce · Arc Testnet</p>
        <h1>The trade record <em>that moves with the goods.</em></h1>
        <p className="hero-copy">Arc Trade gives commercial promises a durable paper trail: issue the terms, name the evidence, and release value only as the record is fulfilled.</p>
        ...
        {authMode === null ? <div className="actions hero-actions"><button disabled={!!busy} onClick={() => { setAuthMode("register"); setMessage(""); }}>Send an agreement <span aria-hidden>↗</span></button><button className="secondary" disabled={!!busy} onClick={() => { setAuthMode("login"); setMessage(""); }}>Login to Arc Trade</button></div> : <section className="panel auth-panel">
          ...
          <p className="notice">{initialSignin.proposalId ? `Proposal ${initialSignin.proposalId} is addressed to this company. Continue with secure sign-in.` : "Your company name locates the profile. Secure sign-in confirms access."}</p>
          ...
          <div className="actions">{authMode === "register" ? <button disabled={!!busy} onClick={() => void signIn("register")}>{busy ? "Creating profile…" : "Proceed"}</button> : <button disabled={!!busy} onClick={() => void signIn("login")}>{busy ? "Signing in…" : "Continue"}</button>}<button className="secondary" disabled={!!busy} onClick={() => { setAuthMode(null); setMessage(""); }}>Back</button></div>
        </section>}
      ...
      <aside className="hero-aside" aria-label="Arc Trade workflow">
        ...
        <p className="eyebrow">The flow</p><ol><li><span className="flow-marker"><i /></span><div><strong>Draft a proposal</strong><small>Private counterparty invitation</small></div></li><li><span className="flow-marker"><i /></span><div><strong>Agree terms</strong><small>Document milestones and response windows</small></div></li><li><span className="flow-marker"><i /></span><div><strong>Settle evidence</strong><small>Funds release when the work is confirmed</small></div></li></ol>
        <div className="aside-footer"><span>Registry-backed</span><strong>AT</strong></div>
      </aside>
    </section>
    <section className="how-it-works" id="how-it-works"><div><p className="eyebrow">How Arc Trade works</p><h2>A paper trail with a pulse.</h2></div><div className="how-copy"><p>Arc Trade turns the messy middle of documentary commerce into a shared, time-bound record. The terms are visible, the evidence has a deadline, and every approved milestone leaves an accountable trail.</p><a href="#principles">Read the operating principles <span aria-hidden>↘</span></a></div></section>
    <section className="principles" id="principles"><article><span>01</span><h3>One shared record</h3><p>Every agreement carries one reference, one set of terms, and one place for both companies to respond.</p></article><article><span>02</span><h3>Proof before release</h3><p>Documentary evidence and response windows make the expected proof visible before value moves.</p></article><article><span>03</span><h3>Time keeps the terms</h3><p>Deadlines, dispute windows, and settlement rules continue to operate even when a participant is offline.</p></article></section>
  </main>;

  if (window.location.pathname.startsWith("/agreements")) return <CommercialWorkflow session={session} onSignOut={signOut} />;
...
function Dashboard(props: { session: EmbeddedWalletSession; company: Company | null; profileHint: string; openProposals: Proposal[]; myProposals: Proposal[]; agreements: AgreementRecord[]; busy: string; message: string; proposalLink: string; recipientCompany: string; agreementLabel: string; proposalDescription: string; proposalVisibility: ProposalVisibility; sellerCommitmentHours: string; buyerResponseMinutes: string; disputeWindowMinutes: string; proposalLifetimeHours: string; joinId: string; setRecipientCompany: (value: string) => void; setAgreementLabel: (value: string) => void; setProposalDescription: (value: string) => void; setProposalVisibility: (value: ProposalVisibility) => void; setSellerCommitmentHours: (value: string) => void; setBuyerResponseMinutes: (value: string) => void; setDisputeWindowMinutes: (value: string) => void; setProposalLifetimeHours: (value: string) => void; setJoinId: (value: string) => void; create: () => void; join: () => void; removeExpired: (proposal: Proposal) => void; restoreProfile: (name: string) => void; select: (record: AgreementRecord) => void; signOut: () => void; }) {
  ...
  return <main className="shell workspace-shell">
    ...
    <section className="workspace-hero"><div><p className="eyebrow">Registry / documentary desk</p><h1>Good to see you, <span>{props.company?.name || "your company"}.</span></h1><p>Keep the terms, evidence, and live settlement record for every serious commercial promise in one operating desk.</p></div><div className="workspace-metrics"><div><strong>{issued.filter(proposal => proposal.status !== "OPEN").length}</strong><span>concluded drafts</span></div><div><strong>{issued.length}</strong><span>proposals drafted</span></div><div><strong>{props.agreements.length}</strong><span>live agreements</span></div></div></section>
    {received.length > 0 && <ActivityPanel title="Received proposals" eyebrow="Needs your attention" proposals={received} props={props} open={setProposalForJoin} removeExpired={props.removeExpired} />}
    <section className="panel agreements-panel"><div className="section-head"><div><p className="eyebrow">Settlement registry</p><h2>Live agreements</h2></div><code>{ref(props.session.address)}</code></div>{props.agreements.length === 0 ? <p className="empty-state">No live agreements yet.</p> : <div className="agreement-list">{props.agreements.map(record => <button className="agreement-row" key={record.id} onClick={() => props.select(record)}><span className="agreement-index">{record.buyer.toLowerCase() === props.session.address.toLowerCase() ? "Buyer" : "Seller"}</span><strong>{ref(record.id)}</strong><code>{ref(record.escrow)}</code><span aria-hidden>→</span></button>)}</div>}</section>
    <FundingPanel address={props.session.address} />
    {issued.length > 0 && <ActivityPanel title="Your proposals" eyebrow="Proposals drafted" proposals={issued} props={props} open={setProposalForJoin} removeExpired={props.removeExpired} />}
    <section className="workflow-grid"><div className="panel create-panel"><p className="eyebrow">Draft a proposal</p><h2>Commercial terms</h2><p className="notice">Choose a private counterparty invitation or place an offer on the board.</p><a className="primary-button" href="/agreements/new">Create commercial agreement</a></div></section>
    <section className="panel board-panel" id="board"><div className="section-head"><div><p className="eyebrow">Global registry / beta</p><h2>Proposal board</h2></div><span className="pill">Coming soon</span></div><p className="notice board-intro">Public offers will appear here in a future release.</p></section>

  </main>;
}

function ActivityPanel({ title, eyebrow, proposals, props, open, removeExpired }: { title: string; eyebrow: string; proposals: Proposal[]; props: { busy: string }; open: (proposal: Proposal) => void; removeExpired: (proposal: Proposal) => void }) {
