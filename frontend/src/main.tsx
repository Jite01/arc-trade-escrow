...
function App() {
  ...
  if (!session) return <main className="shell landing-shell">
    <header className="site-header"><div className="brand"><span className="mark">AT</span><span>Arc<span>Trade</span></span></div><button className="secondary" disabled={!!busy} onClick={() => { setAuthMode("login"); setMessage(""); }}>{busy ? "Working…" : "Login"}</button></header>
    <section className="hero-grid">
      ...
      <aside className="hero-aside" aria-label="Arc Trade workflow">
        ...
        <p className="eyebrow">The flow</p><ol><li><span>01</span><div><strong>Issue proposal</strong><small>Public board or named counterparty</small></div></li><li><span>02</span><div><strong>Agree terms</strong><small>Document milestones and response windows</small></div></li><li><span>03</span><div><strong>Settle evidence</strong><small>Funds release when the work is confirmed</small></div></li></ol>
...
function Dashboard(props: { session: EmbeddedWalletSession; company: Company | null; openProposals: Proposal[]; myProposals: Proposal[]; agreements: AgreementRecord[]; busy: string; message: string; proposalLink: string; recipientCompany: string; agreementLabel: string; proposalDescription: string; proposalVisibility: ProposalVisibility; sellerCommitmentHours: string; buyerResponseMinutes: string; disputeWindowMinutes: string; proposalLifetimeHours: string; joinId: string; setRecipientCompany: (value: string) => void; setAgreementLabel: (value: string) => void; setProposalDescription: (value: string) => void; setProposalVisibility: (value: ProposalVisibility) => void; setSellerCommitmentHours: (value: string) => void; setBuyerResponseMinutes: (value: string) => void; setDisputeWindowMinutes: (value: string) => void; setProposalLifetimeHours: (value: string) => void; setJoinId: (value: string) => void; create: () => void; join: () => void; select: (record: AgreementRecord) => void; signOut: () => void; }) {
  ...
  const setProposalForJoin = (proposal: Proposal) => { props.setJoinId(proposal.id); window.scrollTo({ top: document.body.scrollHeight, behavior: "smooth" }); };
  return <main className="shell workspace-shell">
    <header className="site-header"><div className="brand"><span className="mark">AT</span><span>Arc<span>Trade</span></span></div><div className="account"><strong>{props.company?.name || "Company profile"}</strong><span>{ref(props.session.address)}</span><button className="quiet" onClick={props.signOut}>Sign out</button></div></header>
    {props.message && <div className="error">{props.message}</div>}
    {props.proposalLink && <section className="panel invitation"><div><p className="eyebrow">{props.proposalVisibility === "PRIVATE" ? "Private proposal ready" : "Proposal published to the Arc Trade board"}</p><h2>{props.proposalVisibility === "PRIVATE" ? "Share this invitation" : "Your registry reference"}</h2><p className="notice">{props.proposalVisibility === "PRIVATE" ? "The link includes the named recipient and this proposal’s global registry reference." : "Anyone signed in to Arc Trade can open this proposal using this reference."}</p></div><div className="share-row"><code>{props.proposalLink}</code><button onClick={() => void copyProposalLink()}>{props.proposalVisibility === "PRIVATE" ? "Copy invite link" : "Copy reference"}</button></div></section>}
    <section className="workspace-hero"><div><p className="eyebrow">Registry / workspace</p><h1>Good to see you, <span>{props.company?.name || "there"}.</span></h1><p>Manage your proposals, counterparty invitations, and live settlement agreements from one operating desk.</p></div><div className="workspace-metrics"><div><strong>{props.openProposals.length}</strong><span>open to Arc Trade</span></div><div><strong>{props.myProposals.length}</strong><span>your proposals</span></div><div><strong>{props.agreements.length}</strong><span>live agreements</span></div></div></section>
    ...
    {props.myProposals.length > 0 && <section className="panel board-panel"><div className="section-head"><div><p className="eyebrow">Your activity</p><h2>Issued and received</h2></div><span className="pill">{props.myProposals.length}</span></div><div className="proposal-list">{props.myProposals.map(proposal => <article className="public-proposal" key={proposal.id}><div className="proposal-ref"><code>{proposal.id}</code><span>{proposal.visibility === "PUBLIC" ? "Public board" : `Private · ${proposal.recipientCompany || "recipient"}`}</span></div><div className="proposal-main"><div><h3>{proposal.title}</h3><small>{proposal.status} · {proposal.agreementId ? `Agreement ${ref(proposal.agreementId)}` : "Awaiting agreement deployment"}</small></div><button className="secondary" onClick={() => setProposalForJoin(proposal)}>{proposal.status === "OPEN" ? "Open" : proposal.agreementId ? "View agreement" : "Deploy agreement"}</button></div></article>)}</div></section>}
    <section className="workflow-grid"><div className="panel create-panel"><p className="eyebrow">Issue a proposal</p><h2>Set the commercial terms.</h2><p className="notice">Choose a private counterparty invitation or place an offer on the public board.</p><label className="field-label">Visibility<select aria-label="Proposal visibility" value={props.proposalVisibility} onChange={event => props.setProposalVisibility(event.target.value as ProposalVisibility)}><option value="PRIVATE">Private invitation</option><option value="PUBLIC">Public proposal</option></select></label>{props.proposalVisibility === "PRIVATE" && <label className="field-label">Recipient company<input placeholder="e.g. Northstar Logistics" value={props.recipientCompany} onChange={event => props.setRecipientCompany(event.target.value)} /></label>}<label className="field-label">Proposal title<input placeholder="e.g. Coffee shipment — Lagos to Accra" value={props.agreementLabel} onChange={event => props.setAgreementLabel(event.target.value)} /></label><label className="field-label">Scope and evidence<textarea placeholder="Describe the goods, documents, and proof required." value={props.proposalDescription} onChange={event => props.setProposalDescription(event.target.value)} /></label><div className="window-grid"><label>Response window <small>hours</small><input type="number" min="1" max="168" value={props.sellerCommitmentHours} onChange={event => props.setSellerCommitmentHours(event.target.value)} /></label><label>Negotiation <small>minutes</small><input type="number" min="1" max="1440" value={props.buyerResponseMinutes} onChange={event => props.setBuyerResponseMinutes(event.target.value)} /></label><label>Dispute window <small>minutes</small><input type="number" min="1" max="10080" value={props.disputeWindowMinutes} onChange={event => props.setDisputeWindowMinutes(event.target.value)} /></label><label>Open for <small>hours</small><input type="number" min="1" max="168" value={props.proposalLifetimeHours} onChange={event => props.setProposalLifetimeHours(event.target.value)} /></label></div><button disabled={!!props.busy} onClick={props.create}>{props.busy === "createAgreement" ? "Publishing proposal…" : "Publish proposal"}</button></div>
