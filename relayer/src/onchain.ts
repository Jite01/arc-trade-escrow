...
export class EthersSettlementExecutor implements SettlementExecutor {
  ...
  public async authorize(input:BurnIntentAuthorization, escrowAddress:string):Promise<BurnIntentAuthorizationResult>{ const escrow=new Contract(escrowAddress,["function authorizeBurnIntent(uint256,uint256,uint256,bytes32) returns (bytes32)"],this.signer); const tx=await escrow.authorizeBurnIntent(input.settlementIndex,input.maxBlockHeight,input.maxFee,input.salt); const receipt=await tx.wait(); return {burnIntentHash:burnIntentHash(input.burnIntentRequest),authorizationTxHash:receipt.hash}; }
